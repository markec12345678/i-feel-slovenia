...
export const HeroSection: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {/* Content - z temnim ozadjem za berljivost */}
      <motion.div
        className="relative z-10 max-w-5xl mx-auto"
        initial={shouldAnimate ? { opacity: 0 } : { opacity: 1 }}
        animate={shouldAnimate ? { opacity: 1, transition: { staggerChildren: 0.12, delayChildren: 0.15 } } : { opacity: 1 }}
      >
        {/* 🎨 Temen wrapper za berljivost teksta na svetli sliki */}
        <div className="bg-black/70 backdrop-blur-md px-6 py-8 rounded-2xl border border-white/10 text-center">
          {/* Badge */}
          <motion.div
          ...
          {/* Subheadline – Inter */}
          <motion.p
            className="font-body text-lg md:text-xl text-secondary mb-6 max-w-2xl mx-auto leading-relaxed drop-shadow-md"
          ...
          <motion.div
            className="flex flex-wrap items-center justify-center gap-6 text-secondary/90"
            initial={shouldAnimate ? { opacity: 0 } : { opacity: 1 }}
